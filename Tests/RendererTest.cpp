    ...
    int nCmdShow)
{
    ...
    vertexBuffer[0] = Vertex(Vector3f(-1, -1, -1), Vector3f(1.0f, 0.0f, 0.0f)); // 0: 左-下-前 (红色)
    vertexBuffer[1] = Vertex(Vector3f(1, -1, -1), Vector3f(0.0f, 1.0f, 0.0f));  // 1: 右-下-前 (绿色)
    vertexBuffer[2] = Vertex(Vector3f(1, 1, -1), Vector3f(0.0f, 0.0f, 1.0f));   // 2: 右-上-前 (蓝色)
    vertexBuffer[3] = Vertex(Vector3f(-1, 1, -1), Vector3f(1.0f, 1.0f, 0.0f));  // 3: 左-上-前 (黄色)
    vertexBuffer[4] = Vertex(Vector3f(-1, -1, 1), Vector3f(1.0f, 0.0f, 1.0f));  // 4: 左-下-后 (紫色)
    vertexBuffer[5] = Vertex(Vector3f(1, -1, 1), Vector3f(0.0f, 1.0f, 1.0f));   // 5: 右-下-后 (青色)
    vertexBuffer[6] = Vertex(Vector3f(1, 1, 1), Vector3f(1.0f, 1.0f, 1.0f));    // 6: 右-上-后 (白色)
    vertexBuffer[7] = Vertex(Vector3f(-1, 1, 1), Vector3f(0.5f, 0.5f, 0.5f));   // 7: 左-上-后 (灰色)
